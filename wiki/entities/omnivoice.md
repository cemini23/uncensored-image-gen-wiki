...
Cross-wiki stub routed from `@osint-wiki/entities/tools/omnivoice.md` during ingest.
What prompted this page + which sources synthesize into it — fill in on next
ingest pass.

## Narrative

Zero-shot voice cloning across 600+ languages using a diffusion language model architecture. Apache-2.0 license, 5.9k stars, arXiv-backed. Pure Python/PyTorch with CMU dictionary pronunciation control and expressive non-verbal tags. Adopt tier per K45 v3 multi-wiki tool eval — compatible with existing ComfyUI GPU hardware.
